...
def compute(data):
    return len(list(l for l in data.split("\n") if is_valid(l)))


def compute2(data):
    return len(list(l for l in data.split("\n") if is_valid_with_anagrams(l)))


def equals_last(last, current):
    if last is True or last == current:
        return True
    else:
        return current
...
def is_valid(phrase):
    return functools.reduce(equals_last, sorted(phrase.split(" "))) is not True


def is_valid_with_anagrams(phrase):
    return functools.reduce(equals_last, sorted(sorted(word) for word in phrase.split(" "))) is not True
...
class DayTest(unittest.TestCase):
    def test_count_valid(self):
        input = dedent("""\
            ...
            bb bb
            cc""")
        assert_that(compute(input), is_(2))
...
class ValidPassTest(unittest.TestCase):
    def test_valid1(self):
        assert_that(is_valid("aa bb cc dd ee"), is_(True))

    def test_valid2(self):
        assert_that(is_valid("aa bb cc dd aa"), is_(False))

    def test_valid3(self):
        assert_that(is_valid("aa bb cc dd aaa"), is_(True))

    def test_hm(self):
        assert_that(is_valid('zok ghgp zok lmk'), is_(False))


class AnagramValidPassTest(unittest.TestCase):
    def test_valid1(self):
        assert_that(is_valid_with_anagrams("abcde fghij"), is_(True))

    def test_valid2(self):
        assert_that(is_valid_with_anagrams("abcde xyz ecdab"), is_(False))

    def test_valid3(self):
        assert_that(is_valid_with_anagrams("a ab abc abd abf abj"), is_(True))

    def test_valid4(self):
        assert_that(is_valid_with_anagrams('iiii oiii ooii oooi oooo'), is_(True))

    def test_valid5(self):
        assert_that(is_valid_with_anagrams('oiii ioii iioi iiio'), is_(False))
