...
def compute_easy(data):
    for name in re.findall("^(\w+)\s", data, flags=re.MULTILINE):
        if len(re.findall("\\b{}\\b".format(name), data, flags=re.MULTILINE)) == 1:
            return name
...
def compute2(data):
    ...
    try:
        ...
        return e.expected_weight


class Unbalanced(Exception):
    def __init__(self, prog, expected_weight):
        self.prog = prog
...
def get_weight(root):
    weight, children = (root.weight, [(c, _total(get_weight(c))) for c in root.children])

    groups = sorted(((i, list(l))
                     for i, l in groupby(sorted(children, key=lambda e: e[1]),
                                         key=lambda e: e[1])),
                    key=lambda e: len(e[1]))

    if len(groups) > 1:
        prog = groups[0][1][0][0]
        raise Unbalanced(groups[0][1][0], prog.weight - (groups[0][0] - groups[1][0]))

    return weight, [c[1] for c in children]


def parse(data):
    all_progs = {}
    unassigned_progs = {}
    for name, weight in re.findall("^(\w+)\s\((\d+)\)", data, flags=re.MULTILINE):
        all_progs[name] = unassigned_progs[name] = Prog(name, int(weight), [])

    for name, associations in re.findall("^(\w+)\s\(\d+\) -> ([^$\n]*)", data, flags=re.MULTILINE):
        all_progs[name].children = [unassigned_progs.pop(a) for a in associations.split(', ')]

    return list(unassigned_progs.values())[0]
...
class DayTest(unittest.TestCase):
    def test_example(self):
        input = dedent("""\
            ...
            gyxo (61)
            cntj (57)""")
        assert_that(compute(input), is_("tknk"))
...
class Day2Test(unittest.TestCase):
    def test_example(self):
        input = dedent("""\
            ...
            gyxo (61)
            cntj (57)""")
        assert_that(compute2(input), is_(60))


class GetWeightTest(unittest.TestCase):
    def test_simple(self):
        weight = get_weight(Prog("a", 1, []))
        assert_that(weight, is_((1, [])))
    ...
    def test_with_children(self):
        ...
        assert_that(weight, is_((1, [1, 1, 1])))

    def test_with_2_levels_of_children(self):
        weight = get_weight(Prog("a", 1, [
            Prog("b", 4, []),
            Prog("c", 1, [
                Prog("e", 1, []),
        ...
        assert_that(weight, is_((1, [4, 4, 4])))

    def test_unbalanced(self):
        with self.assertRaises(Unbalanced) as expect:
            get_weight(Prog("a", 1, [
                Prog("b", 1, []),
                Prog("c", 2, []),
                Prog("d", 1, []),
        ...
        assert_that(expect.exception.expected_weight, is_(1))

    def test_unbalanced_in_sub(self):
        with self.assertRaises(Unbalanced) as expect:
            get_weight(Prog("a", 1, [
                Prog("b", 5, []),
                Prog("c", 1, [
        ...
        assert_that(expect.exception.expected_weight, is_(2))
