...
def compute(data, similarity=10, confidence_level=100):
    return trend_finder(ClosestLongTermParticleFinder(parse(data), similarity=similarity), confidence_level)


def compute2(data, confidence_level=100):
    return trend_finder(CollisionFree(parse(data)), confidence_level)


def trend_finder(state, confidence_level):
    current = state.initial()
    similar = 0

    while similar < confidence_level:
        last = current
        current = state.evolve()

        if state.compare(last, current):
            similar += 1
            print("Similar={}; state={}".format(similar, state.state(current)))
        else:
            similar = 0
...
class ClosestLongTermParticleFinder:
    def __init__(self, particles, similarity):
    ...
    def evolve(self):
        dist_for_particles = [(particle.distance(), index) for index, particle in enumerate(tick(self.particles))]
        return [particle_index for distance, particle_index in sorted(dist_for_particles, key=lambda e: e[0])]

    def compare(self, last, current):
        return last[:self.similarity] == current[:self.similarity]

    def state(self, current):
        return ",".join(["{}={}".format(p, self.particles[p].position) for p in current[:self.similarity]])

    def result(self, current):
        return current[0]
...
class CollisionFree:
    def __init__(self, particles):
        self.particles = particles
        self.grouping = lambda p: p.position
    ...
    def evolve(self):
        grouped = (list(grouper)
                   for _, grouper in
                   groupby(sorted(tick(self.particles), key=self.grouping), key=self.grouping))

        self.particles = [items[0] for items in grouped if len(items) == 1]
...
class Particle:
    ...
    def __next__(self):
        for a in AXES:
            self.velocity[a] += self.acceleration[a]
...
def parse(data):
    return [Particle(_to_vector(p), _to_vector(v), _to_vector(a))
            for p, v, a in re.findall("^p=<(.*)>, v=<(.*)>, a=<(.*)>$", data, flags=re.MULTILINE)]


def _to_vector(data):
    return [int(e) for e in data.split(",")]
...
class DayTest(unittest.TestCase):
    def test_example(self):
        ...
        assert_that(compute(input), is_(0))

    def test_puzzle(self):
        assert_that(compute(puzzle_input), is_(243))
...
class Day2Test(unittest.TestCase):
    ...
    def test_puzzle(self):
        assert_that(compute2(puzzle_input), is_(648))
...
class ParseTest(unittest.TestCase):
    def test(self):
        ...
        particle1, particle2 = parse(input)

        assert_that(particle1.start, is_([3, 0, 0]))
        assert_that(particle1.position, is_([3, 0, 0]))
        assert_that(particle1.velocity, is_([2, 0, 0]))
        assert_that(particle1.acceleration, is_([-1, 0, 0]))

        assert_that(particle2.start, is_([4, 0, 0]))
        assert_that(particle2.position, is_([4, 0, 0]))
        assert_that(particle2.velocity, is_([0, 0, 0]))
        assert_that(particle2.acceleration, is_([-2, 0, 0]))


class ParticleTest(unittest.TestCase):
    def test(self):
        particles = Particle([1, 1, 1], [1, 1, 1], [1, 1, 1])

        particle = next(particles)
        assert_that(particle.distance(), is_(9))
        assert_that(particle.position, is_([3, 3, 3]))
        assert_that(particle.velocity, is_([2, 2, 2]))

        particle = next(particles)
        assert_that(particle.distance(), is_(18))
        assert_that(particle.position, is_([6, 6, 6]))
        assert_that(particle.velocity, is_([3, 3, 3]))
