...
def compute(data):
    return sum(max(line) - min(line) for line in parse(data))


def compute2(data):
    return sum(find_evenly(line) for line in parse(data))
...
class DayTest(unittest.TestCase):
    def test_difference_min_max(self):
        spreadsheet = dedent("""\
            5 1 9 5
            7 5 3
            2 4 6 8""")
        assert_that(compute(spreadsheet), is_(18))


def find_evenly(line):
    for e1 in line:
        for e2 in line:
            diff = e1 / e2
            if diff > 1 and diff == floor(diff):
                return int(diff)
...
class Day2Test(unittest.TestCase):
    def test_evenly(self):
        spreadsheet = dedent("""\
            ...
            9 4 7 3
            3 8 6 5""")
        assert_that(compute2(spreadsheet), is_(9))

    def test_find_evenly(self):
        assert_that(find_evenly([5, 9, 2, 8]), is_(4))
        assert_that(find_evenly([9, 4, 7, 3]), is_(3))
