...
def compute(decrypted_name, data):
    for line in data.strip().split("\n"):
        match = re.match("(.*)-(\d+)\[(\w+)\]", line)
        name, sector_id, checksum = match.groups()

        print("- {}".format(decrypt(name, int(sector_id))))
        if is_real(name, checksum) and decrypt(name, int(sector_id)) == decrypted_name:
            return sector_id
...
def is_real(name, checksum):
    ...
    for i in range(0, len(letters)):
        if letters[i] not in bank:
            bank[letters[i]] = 0

        bank[letters[i]] -= 1

    top_letters = "".join([str(k) for k, v in sorted(bank.items(), key=lambda e: (e[1], e[0]))])
    return top_letters[:5] == checksum
...
def decrypt(name, shift):
    return "".join([chr((ord(letter) - A + shift) % 26 + A) if letter != SPACE else ' '
                    for letter in name])


class ComputeTest(unittest.TestCase):
    def test_is_real(self):
        assert_that(is_real("aaaaa-bbb-z-y-x", "abxyz"), is_(True))
        assert_that(is_real("a-b-c-d-e-f-g-h", "abcde"), is_(True))
        assert_that(is_real("not-a-real-room", "oarel"), is_(True))
        assert_that(is_real("totally-real-room", "decoy"), is_(False))
        assert_that(is_real("qzmt-zixmtkozy-ivhz", "zimth"), is_(True))

    def test_decrypt(self):
        assert_that(decrypt("z", 1), is_("a"))
        assert_that(decrypt("a", 26), is_("a"))
        assert_that(decrypt("qzmt-zixmtkozy-ivhz", 343), is_("very encrypted name"))

    def test_1(self):
        result = compute("very encrypted name", dedent("""
            a-b-c-d-e-f-g-h-987[abcde]
            very-encrypted-name-26[devoy]
