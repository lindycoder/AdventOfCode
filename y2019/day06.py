...
def compute(data):
    orbits = {'COM': 0}
    treat = [l.split(')') for l in data.strip().splitlines()]
    while len(treat) > 0:
        obj, sat = treat.pop(0)
        if obj in orbits:
            orbits[sat] = orbits[obj] + 1
        else:
            treat.append((obj, sat))
...
def compute2(data):
    ...
    finder = astar.pathfinder(neighbors=lambda c: neighbors[c], distance=lambda *_: 1, cost=astar.fixed_cost(1))
