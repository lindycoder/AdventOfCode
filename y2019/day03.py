...
def compute2(data):
    ...
    intersections_steps = [steps_to1[i] + steps_to2[i]
...
def parse_path(data):
    return [(mapping[d[0]], int(d[1:])) for d in data.split(',')]


def crawl(wire_moves):
    position = ORIGIN

    points = {position}
    steps = 0
    steps_to = {position: steps}
    for direction, number in wire_moves:
        for _ in range(number):
            position += direction.value
            points.add(position)
            steps += 1
            if position not in steps_to:
                steps_to[position] = steps
    return points, steps_to
