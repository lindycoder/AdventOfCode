...
class Input:
    ...
    @classmethod
    def parse(cls, input: str) -> Iterable['Input']:
        yield from (
            cls(*line.split(': '))
...
def compute(data):
    return len(list(filter(None, map(validate, Input.parse(data)))))


def compute2(data):
    validator = partial(validate, to_regex=policy_to_regex2)
    return len(list(filter(None, map(validator, Input.parse(data)))))


_POLICY_REGEX = re.compile(r'^(\d+)-(\d+) (\w)$')

_REGEX_TEMPLATE = '^([^{letter}]*{letter}[^{letter}]*){{{min},{max}}}$'

def policy_to_regex(policy: str) -> str:
    mi, ma, letter = _POLICY_REGEX.match(policy).groups()
    return _REGEX_TEMPLATE.format(
        letter=letter,
        min=mi,
        max=ma
    )
...
_REGEX_TEMPLATE2 = '^((.{{{min}}}{letter}.{{{max}}}[^{letter}])|(.{{{min}}}[^{letter}].{{{max}}}{letter})).*$'

def policy_to_regex2(policy: str) -> str:
    mi, ma, letter = _POLICY_REGEX.match(policy).groups()
    return _REGEX_TEMPLATE2.format(
        letter=letter,
        min=int(mi) - 1,
        max=int(ma) - int(mi) - 1,
    )
...
def validate(input: Input, to_regex=policy_to_regex):
    return re.match(to_regex(input.policy), input.password)
...
@pytest.mark.parametrize('val,to_regex,expect', [
    (Input('1-3 a', 'abcde'), policy_to_regex, True),
    (Input('1-3 a', 'abcde'), policy_to_regex2, True),
    (Input('1-3 b', 'cdefg'), policy_to_regex, False),
    (Input('1-3 b', 'cdefg'), policy_to_regex2, False),
    (Input('2-9 c', 'ccccccccc'), policy_to_regex, True),
    (Input('2-9 c', 'ccccccccc'), policy_to_regex2, False),
])
def test_validate(val,to_regex,expect):
    assert_that(bool(validate(val, to_regex)), is_(expect))
...
def test_policy_to_regex(val, expect):
    assert_that(policy_to_regex(val), is_(expect))


@pytest.mark.parametrize('val,expect', [
    ('1-3 a', '^((.{0}a.{1}[^a])|(.{0}[^a].{1}a)).*$'),
    ('2-9 c', '^((.{1}c.{6}[^c])|(.{1}[^c].{6}c)).*$'),
    ('12-13 l', '^((.{11}l.{0}[^l])|(.{11}[^l].{0}l)).*$'),
])
def test_policy_to_regex2(val, expect):
    assert_that(policy_to_regex2(val), is_(expect))
