...
def compute(data):
    return len(list(filter(lambda e: len(e) == 7, parse(data))))


IS_YEAR = partial(re.match, r'^\d{4}$')
EYE_COLORS = ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth')
validators = {
    'byr': lambda v: bool(IS_YEAR(v) and 1920 <= int(v) <= 2002),
    'iyr': lambda v: bool(IS_YEAR(v) and 2010 <= int(v) <= 2020),
    'eyr': lambda v: bool(IS_YEAR(v) and 2020 <= int(v) <= 2030),
    'hgt': lambda v: bool(
        (re.match(r'^\d{3}cm$', v) and 150 <= int(v[:-2]) <= 193)
        or
        (re.match(r'^\d{2}in$', v) and 59 <= int(v[:-2]) <= 76)
    ),
...
def validate_passport(passport_data):
    return all(validators[k](v) for k, v in passport_data.items())


def compute2(data):
    return len(list(filter(lambda e: len(e) == 7 and validate_passport(e),
                           parse(data))))


def parse(data: str, skip_fields=('cid',)) -> Iterable[Mapping['str', 'str']]:
    for passport_data in data.strip().split(('\n\n')):
        yield {
            k: v for k, v in split_passport(passport_data)
            if k not in skip_fields
        }


def split_passport(passport_data):
    return map(lambda e: e.split(':'), re.split(r'[\n\s]', passport_data))
...
def test_split_passport(val, expect):
    assert_that(list(split_passport(val)), is_(expect))
...
def test_validators(field, value, match):
    assert_that(validators[field](value), is_(match))
