...
bag_re = re.compile(r'^(\w+\s\w+) bags? contain(.*)\.$')
content_re = re.compile(r'^ (\d) (\w+\s\w+) bags?$')

@dataclass(frozen=True)
class Bag:
    color: str
    content: Mapping[str, int]

    @classmethod
    def from_string(cls, data: str) -> 'Bag':
        color, content = bag_re.match(data).groups()
        return cls(color, {
            r.group(2): int(r.group(1))
            for r in filter(None, map(content_re.match, content.split(',')))
        })


def compute(data, lookup='shiny gold'):
    parents_of = defaultdict(list)
    for bag in map(Bag.from_string, data.strip().splitlines()):
        for child in bag.content.keys():
            parents_of[child].append(bag.color)

    parents = parents_of[lookup]
    unique_parents = set()
    while parents:
        parent = parents.pop(0)
        unique_parents.add(parent)
        parents.extend(parents_of[parent])

    return len(unique_parents)


def compute2(data, lookup='shiny gold'):
    bags = {bag.color: bag
            for bag in map(Bag.from_string, data.strip().splitlines())}

    to_compile = [(1, bags[lookup])]
    bag_count = 0
    while to_compile:
        bag_qty, bag = to_compile.pop(0)
        for color, qty in bag.content.items():
            bag_count += bag_qty * qty
            to_compile.append((bag_qty * qty, bags[color]))

    return bag_count

@pytest.mark.parametrize('val,expect', [
    ('light red bags contain 1 bright white bag, 2 muted yellow bags.', Bag('light red', content={'bright white': 1, 'muted yellow': 2})),
    ('bright white bags contain 1 shiny gold bag.', Bag('bright white', content={'shiny gold': 1})),
    ('faded blue bags contain no other bags.', Bag('faded blue', content={})),
])
def test_parse_bag(val, expect):
    assert_that(Bag.from_string(val), is_(expect))
