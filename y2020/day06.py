...
def compute(data):
    return sum(len(parse_group(g)) for g in data.strip().split('\n\n'))


def compute2(data):
    return sum(len(parse_group2(g)) for g in data.strip().split('\n\n'))


def parse_group(data):
    return set(e for line in data.splitlines() for e in line)


def parse_group2(data):
    return reduce(
        lambda final, s: final.intersection(s),
        map(set, data.splitlines()),
...
def test_parse_group(val, expect):
    assert_that(parse_group(val), expect)
...
def test_parse_group2(val, expect):
    assert_that(parse_group2(val), expect)
