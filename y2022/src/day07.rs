...
pub fn version2(input: Option<String>) -> String {
    let data = input.unwrap_or_else(puzzle_input);
    let sizes = compute_sizes(data);
    let root_size = sizes["root"];
    let unused_space = 70000000 - root_size;
    let needed_space = 30000000 - unused_space;
    sizes
        .values()
        .filter(|e| **e >= needed_space)
        .min()
        .unwrap()
        .to_string()
}

fn compute_sizes(data: String) -> HashMap<String, usize> {
    ...
    for line in data.lines() {
        if line.starts_with("$ cd") {
            ...
            let first = line.split_once(' ').unwrap().0;
            let file_size = first.parse::<usize>().unwrap();
            for i in 0..path.len() {
                let key = path[0..(i + 1)].join("/");
                if !sizes.contains_key(&key) {
                    sizes.insert(key.to_string(), file_size);
                } else {
                    *sizes.get_mut(&key).unwrap() += file_size;
                }
            }
        }
    }
...
#[cfg(test)]
mod tests {
    use crate::day07::{version1, version2};
...
75000 file.txt";
        let result = version1(Some(input.to_string()));
        assert_eq!(result, "300000"); // dir1 + common_name + dir2 + common_name
    }
