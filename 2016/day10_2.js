...
function compute(lookup, input, done) {
    ...
    done(factory.objects[OUTPUT][0].value * factory.objects[OUTPUT][1].value * factory.objects[OUTPUT][2].value)
}
...
describe('Output', function () {
    it('should gather', function () {
        var output = new Output(111, function() {});

        output.receive(5);
        expect(output.value).to.equal(5);
    });
});
