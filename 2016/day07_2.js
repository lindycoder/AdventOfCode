...
function compute(data) {
    var ips = lines = data.split("\n");

    return ips.reduce((total, ip) => total += isSSL(ip) ? 1 : 0, 0)
}
...
function isSSL(ip) {
    ...
    for (i = 0; i < parts.length; i+= 2) {
        supernetABAs = supernetABAs.concat(getABAs(parts[i]));
    }

    for (i = 1; i < parts.length; i+= 2) {
        abas = getABAs(parts[i]);
        for (var j = 0; j < abas.length; j++) {
            if (supernetABAs.indexOf(flipABA(abas[j])) > -1) {
                return true
            }
        }
    }
...
function getABAs(seq) {
    var challenge,
        abas = [];

    for (var i = 0; i <= seq.length - 3; i++) {
        challenge = seq.substr(i, 3);
        if (challenge.charAt(0) !== challenge.charAt(1) &&
                challenge.charAt(0) === challenge.charAt(2)) {
            abas.push(challenge)
        }
    }
...
function flipABA(aba) {
    return aba.substr(1) + aba.charAt(1);
}

describe('flipABA', function () {
    it('does what it is supposed to', function () {
        expect(flipABA("aba")).to.deep.equal("bab");
    });
});

describe('getABAs', function () {
    it('does what it is supposed to', function () {
        expect(getABAs("aba")).to.deep.equal(["aba"]);
        expect(getABAs("aaa")).to.deep.equal([]);
        expect(getABAs("abab")).to.deep.equal(["aba", "bab"]);
        expect(getABAs("aaabaaa")).to.deep.equal(["aba"]);
        expect(getABAs("aaaaaba")).to.deep.equal(["aba"]);
        expect(getABAs("abaaaaa")).to.deep.equal(["aba"]);
        expect(getABAs("aabaa")).to.deep.equal(["aba"]);
    });
});
...
describe('isSSL', function () {
    ...
    it('false if no reverse in brackes', function () {
        expect(isSSL("xyx[xyx]xyx")).to.equal(false);
    });
    it('ok when after', function () {
        expect(isSSL("aaa[kek]eke")).to.equal(true);
    });
    it('ok when in string', function () {
        expect(isSSL("zazbz[bzb]cdb")).to.equal(true);
    });
    it('should work if reverse in ANY bracket', function () {
        expect(isSSL("ioxoj[asoxogh]a[kek]eke")).to.equal(true);
    });
});

describe('Compute', function () {
    it('should do stuff', function () {
        var result = compute(`aba[bab]xyz
xyx[xyx]xyx
aaa[kek]eke
zazbz[bzb]cdb
`);
...
if (run_for_real) {
    ...
    console.log("Result is", compute(`xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
